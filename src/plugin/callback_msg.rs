...
const MSG_TO_MCPDESK_TARGET: &str = "mcpdesk";
...
pub(super) extern "C" fn cb_msg(
    ...
    match &target as _ {
        ...
        MSG_TO_MCPDESK_TARGET => handle_msg_to_mcpdesk(id, content, len),
        _ => PluginReturn::new(
...
fn handle_msg_to_mcpdesk(id: String, content: *const c_void, len: usize) -> PluginReturn {
    let s = early_return_value!(
        std::str::from_utf8(unsafe { std::slice::from_raw_parts(content as _, len) }),
        ERR_CALLBACK_INVALID_MSG,
        "parse msg string"
    );
    let msg_to_mcpdesk = early_return_value!(
        serde_json::from_str::<MsgToMcpdesk>(s),
        ERR_CALLBACK_INVALID_MSG,
        "parse msg '{}'",
        s
    );
    match &msg_to_mcpdesk.r#type as &str {
        MSG_TO_RUSTDESK_SIGNATURE_VERIFICATION => request_plugin_sign(id, msg_to_mcpdesk),
        t => PluginReturn::new(
            errno::ERR_CALLBACK_TARGET_TYPE,
            &format!(
...
fn request_plugin_sign(id: String, msg_to_mcpdesk: MsgToMcpdesk) -> PluginReturn {
    let signature_data = early_return_value!(
        std::str::from_utf8(&msg_to_mcpdesk.data),
        ERR_CALLBACK_INVALID_MSG,
