...
//*************************************************************
//
//  Purpose:    Deletes mcpdesk Test certificates and wrong key stores
//
...
BOOL DeleteMcpdeskTestCertsW_SingleHive(HKEY RootKey, LPWSTR Prefix = NULL) {
...
extern "C" void DeleteMcpdeskTestCertsW() {
	// Current user
	std::wcout << "*** Current User" << std::endl;
	DeleteMcpdeskTestCertsW_SingleHive(HKEY_CURRENT_USER);

	// Local machine (requires admin rights)
	std::wcout << "*** Local Machine" << std::endl;
	DeleteMcpdeskTestCertsW_SingleHive(HKEY_LOCAL_MACHINE);
	...
	for (DWORD Index = 0; ; Index++) {
		...
		DeleteMcpdeskTestCertsW_SingleHive(HKEY_USERS, SubKeyName);
	}
