...
pub fn get_cur_device_string() -> &'static str {
    match IDD_IMPL {
        IDD_IMPL_MCPDESK => MCPDESK_IDD_DEVICE_STRING,
...
pub fn plug_in_headless() -> ResultType<()> {
    match IDD_IMPL {
        IDD_IMPL_MCPDESK => mcpdesk_idd::plug_in_headless(),
...
pub fn get_platform_additions() -> serde_json::Map<String, serde_json::Value> {
    ...
    match IDD_IMPL {
        IDD_IMPL_MCPDESK => {
            let virtual_displays = mcpdesk_idd::get_virtual_displays();
            if !virtual_displays.is_empty() {
                map.insert(
                    "mcpdesk_virtual_displays".into(),
...
pub fn plug_in_monitor(idx: u32, modes: Vec<virtual_display::MonitorMode>) -> ResultType<()> {
    match IDD_IMPL {
        IDD_IMPL_MCPDESK => mcpdesk_idd::plug_in_index_modes(idx, modes),
    ...
    match IDD_IMPL {
        IDD_IMPL_MCPDESK => {
            let indices = if index == IDD_PLUG_OUT_ALL_INDEX {
                mcpdesk_idd::get_virtual_displays()
            } else {
                vec![index as _]
            };
            mcpdesk_idd::plug_out_peer_request(&indices)
        }
...
pub fn plug_in_peer_request(modes: Vec<Vec<virtual_display::MonitorMode>>) -> ResultType<Vec<u32>> {
    match IDD_IMPL {
        IDD_IMPL_MCPDESK => mcpdesk_idd::plug_in_peer_request(modes),
        IDD_IMPL_AMYUNI => {
...
pub fn plug_out_monitor_indices(
    ...
    match IDD_IMPL {
        IDD_IMPL_MCPDESK => mcpdesk_idd::plug_out_peer_request(indices),
        IDD_IMPL_AMYUNI => {
...
pub fn reset_all() -> ResultType<()> {
    match IDD_IMPL {
        IDD_IMPL_MCPDESK => mcpdesk_idd::reset_all(),
...
pub mod mcpdesk_idd {
    ...
    fn get_device_names() -> Vec<String> {
        windows::get_device_names(Some(super::MCPDESK_IDD_DEVICE_STRING))
    }
...
pub mod amyuni_idd {
    ...
    // 2. mcpdesk may crash and restart, but the virtual displays are kept.
    //
    ...
    pub fn plug_out_monitor(index: i32, force_all: bool, force_one: bool) -> ResultType<()> {
        ...
        //
        // 1. mcpdesk plug in 2 virtual displays. (mcpdesk)
        // 2. Other process plug out all virtual displays. (User manually)
        // 3. Other process plug in 1 virtual display. (User manually)
        // 4. mcpdesk plug out all virtual displays in this call. (mcpdesk disconnect)
        //
        // This is not a normal scenario, mcpdesk will plug out virtual display unexpectedly.
